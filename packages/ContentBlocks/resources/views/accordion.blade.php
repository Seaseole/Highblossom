...
<div
    class="cb-accordion"
    data-cb-accordion-id="{{ $accordionId }}"
    data-cb-accordion-multiple="{{ $multipleOpen ? 'true' : 'false' }}"
>
    @foreach ($items as $index => $item)
        ...
        <details
            ...
            data-cb-accordion-item="{{ $itemId }}"
            @if (! $multipleOpen) data-cb-accordion-exclusive @endif
        >
            ...
            <div class="cb-accordion__content">{!! $item['content'] ?? '' !!}</div>
        </details>
    @endforeach
</div>
