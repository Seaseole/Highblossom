...
<?php if ($as !== 'button') { ?>
<a {{ $attributes->class($classes) }} data-flux-link <?php if ($external) { ?>target="_blank" <?php } ?>>{{ $slot }}</a>
<?php } else { ?>
<button {{ $attributes->merge(['class' => $classes, 'type' => 'button']) }} data-flux-link>{{ $slot }}</button>
<?php } ?>
