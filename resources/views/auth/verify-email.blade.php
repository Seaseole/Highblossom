            ...
            <div class="w-full max-w-md relative z-10">
                ...
                <div class="glass-vivid rounded-[2rem] p-8 lg:p-10 animate-fade-in-up delay-100">
                    <div class="mb-8 text-center">
                        <div class="w-16 h-16 bg-gradient-to-br from-[#DC2626]/10 to-[#B91C1C]/10 rounded-full flex items-center justify-center mx-auto mb-6">
                            <svg class="w-8 h-8 text-[#DC2626]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"></path>
                            </svg>
                        </div>
                        <h2 class="text-3xl font-bold text-[#18181B] mb-3 font-headline tracking-tight">Verify Your Email</h2>
                        <p class="text-[#71717A]">We've sent a verification link to your email address. Please click it to activate your account.</p>
                    </div>

                    @if (session('resent'))
                        <div class="mb-6 bg-green-500/10 border border-green-500/20 rounded-2xl p-4 animate-fade-in-up delay-200">
                            <p class="text-sm text-green-600 font-medium">A fresh verification link has been sent to your email address.</p>
                        </div>
                    @endif

                    <div class="space-y-4">
                        <form method="POST" action="{{ route('verification.send') }}" class="animate-fade-in-up delay-200">
                            @csrf
                            <button
                                type="submit"
                        ...
                        <form method="POST" action="{{ route('logout') }}" class="animate-fade-in-up delay-300">
                            @csrf
                            <button
                                type="submit"
                                class="w-full bg-white/50 text-[#71717A] py-4 px-6 rounded-2xl font-bold border border-[#E4E4E7] hover:bg-white/80 hover:text-[#18181B] transition-all duration-300 active:scale-[0.98]"
                            >
                                Logout
                            </button>
                        </form>
