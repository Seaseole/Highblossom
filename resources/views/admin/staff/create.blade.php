    ...
    <div class="p-8 max-w-xl mx-auto space-y-10">
        ...
        <div class="space-y-1">
            <div class="flex items-center gap-3 text-admin-text-muted mb-2">
                ...
                <span class="text-[10px] font-bold uppercase tracking-[0.2em] font-body">Staff / New</span>
            </div>
            <h1 class="text-4xl font-bold tracking-tight text-admin-text font-headline leading-none">Add Staff Member</h1>
            <p class="text-admin-text-muted text-sm">Introduce a new member of your expert team.</p>
        </div>
        ...
        <form action="{{ route('admin.staff.store') }}" method="POST" enctype="multipart/form-data" 
                    ...
                    <div class="relative admin-form-input min-h-[160px] flex flex-col items-center justify-center border-dashed border-2 border-admin-border-subtle cursor-pointer hover:border-admin-accent/50 transition-colors" @click="$refs.photoInput.click()">
                        <template x-if="!imagePreview">
                            <div class="flex flex-col items-center text-center p-6">
                                <svg class="w-8 h-8 text-admin-text-muted mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" /></svg>
                                <span class="text-[10px] font-bold uppercase tracking-widest">Select Photo</span>
                            </div>
                        </template>
                        <template x-if="imagePreview">
            ...
            <div class="pt-6 border-t border-admin-border-subtle flex justify-end gap-3">
                ...
                <button type="submit" class="px-8 py-3 bg-[#DC2626] hover:bg-[#B91C1C] text-white font-bold rounded-xl shadow-lg shadow-[#DC2626]/20 transition-all active:scale-[0.98]">Save Member</button>
            </div>
        </form>
    </div>
</x-layouts::admin>