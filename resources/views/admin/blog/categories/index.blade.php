<x-layouts::admin title="Categories">
    <div class="p-6">
        <div class="admin-section-header">
            <h1 class="admin-section-title">Categories</h1>
            <a href="{{ route('admin.categories.create') }}" class="admin-action-btn admin-action-btn-primary">
                    ...
                    @forelse($categories as $category)
                        <tr class="transition-colors duration-200">
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm font-medium text-admin-text">{{ $category->name }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-admin-text-muted">
                                {{ $category->slug }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <a href="{{ route('admin.categories.edit', $category) }}" class="admin-action-btn admin-action-btn-secondary">
