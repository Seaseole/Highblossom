<x-layouts::admin title="Edit Gallery Item">
    <div class="p-8">
        ...
        <div class="mb-8">
            ...
            <h1 class="text-4xl font-bold text-[#39393c] tracking-tight">Edit Gallery Item</h1>
            <p class="mt-2 text-zinc-500">Update gallery item details and manage content</p>
        </div>

        <form method="POST" action="{{ route('admin.gallery.update', $item) }}" class="max-w-3xl" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-8" x-data="{ 
                ...
                <div class="space-y-6">
                    <div>
                        ...
                        <input type="text" name="title" id="title" value="{{ old('title', $item->title) }}" required class="w-full px-4 py-3 rounded-xl border border-zinc-200 bg-white text-zinc-900 placeholder-zinc-400 focus:border-[#dc2626] focus:ring-4 focus:ring-[#dc2626]/10 transition-all duration-200" placeholder="Gallery item title">
                        @error('title')
                    ...
                    <div>
                        ...
                        <select name="category" id="category" required class="w-full px-4 py-3 rounded-xl border border-zinc-200 bg-white text-zinc-900 focus:border-[#dc2626] focus:ring-4 focus:ring-[#dc2626]/10 transition-all duration-200">
                            ...
                            <option value="heavy_machinery" {{ old('category', $item->category) === 'heavy_machinery' ? 'selected' : '' }}>Heavy Machinery</option>
                            <option value="fleet" {{ old('category', $item->category) === 'fleet' ? 'selected' : '' }}>Fleet</option>
                    ...
                    <div>
                        ...
                        <textarea name="description" id="description" rows="3" class="w-full px-4 py-3 rounded-xl border border-zinc-200 bg-white text-zinc-900 placeholder-zinc-400 focus:border-[#dc2626] focus:ring-4 focus:ring-[#dc2626]/10 transition-all duration-200 resize-none" placeholder="Description of the work done">{{ old('description', $item->description) }}</textarea>
                        @error('description')
                            <p class="mt-2 text-sm text-[#dc2626]">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div class="space-y-6">
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="number" name="latitude" id="latitude" step="any" value="{{ old('latitude', $item->latitude) }}" class="w-full px-4 py-3 rounded-xl border border-zinc-200 bg-white text-zinc-900 placeholder-zinc-400 focus:border-[#dc2626] focus:ring-4 focus:ring-[#dc2626]/10 transition-all duration-200" placeholder="-24.6532">
                            @error('latitude')
                                <p class="mt-2 text-sm text-[#dc2626]">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <input type="number" name="longitude" id="longitude" step="any" value="{{ old('longitude', $item->longitude) }}" class="w-full px-4 py-3 rounded-xl border border-zinc-200 bg-white text-zinc-900 placeholder-zinc-400 focus:border-[#dc2626] focus:ring-4 focus:ring-[#dc2626]/10 transition-all duration-200" placeholder="25.9087">
                            @error('longitude')
                                <p class="mt-2 text-sm text-[#dc2626]">{{ $message }}</p>
                            @enderror
                        </div>
                    ...
                    <div>
                        ...
                        <input type="text" name="location_address" id="location_address" value="{{ old('location_address', $item->location_address) }}" class="w-full px-4 py-3 rounded-xl border border-zinc-200 bg-white text-zinc-900 placeholder-zinc-400 focus:border-[#dc2626] focus:ring-4 focus:ring-[#dc2626]/10 transition-all duration-200" placeholder="123 Main St, Gaborone, Botswana">
                ...
                <div class="space-y-6">
                    <div>
                        ...
                        <input type="number" name="sort_order" id="sort_order" value="{{ old('sort_order', $item->sort_order) }}" min="0" class="w-full px-4 py-3 rounded-xl border border-zinc-200 bg-white text-zinc-900 placeholder-zinc-400 focus:border-[#dc2626] focus:ring-4 focus:ring-[#dc2626]/10 transition-all duration-200" placeholder="Lower numbers appear first">
                        @error('sort_order')
                            <p class="mt-2 text-sm text-[#dc2626]">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="flex items-center gap-3 p-4 bg-zinc-50 rounded-xl border border-zinc-200">
                        ...
                        <input type="checkbox" name="is_featured" id="is_featured" value="1" {{ old('is_featured', $item->is_featured) ? 'checked' : '' }} class="h-5 w-5 text-[#dc2626] focus:ring-[#dc2626] focus:ring-offset-0 border-zinc-300 rounded transition-all duration-200 cursor-pointer">
                    ...
                    <div class="flex items-center gap-3 p-4 bg-zinc-50 rounded-xl border border-zinc-200">
                        ...
                        <input type="checkbox" name="is_active" id="is_active" value="1" {{ old('is_active', $item->is_active) ? 'checked' : '' }} class="h-5 w-5 text-[#dc2626] focus:ring-[#dc2626] focus:ring-offset-0 border-zinc-300 rounded transition-all duration-200 cursor-pointer">
