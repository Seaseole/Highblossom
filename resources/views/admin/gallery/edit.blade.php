<x-layouts::admin title="Edit Gallery Item">
    <div class="p-6">
        <div class="admin-section-header">
            <h1 class="admin-section-title">Edit Gallery Item</h1>
            ...
            <div class="space-y-8" x-data="{ 
                ...
                <div class="bg-admin-surface-alt border border-admin-border rounded-xl p-6 space-y-4">
                    ...
                    <div>
                        ...
                        <input type="text" name="title" id="title" value="{{ old('title', $item->title) }}" required class="w-full admin-form-input" placeholder="Gallery item title">
                        @error('title')
                    ...
                    <div>
                        ...
                        <textarea name="description" id="description" rows="3" class="w-full admin-form-input resize-none" placeholder="Description of the work done">{{ old('description', $item->description) }}</textarea>
                        @error('description')
                            <p class="mt-1 text-sm text-[#DC2626]">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div class="bg-admin-surface-alt border border-admin-border rounded-xl p-6 space-y-4">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="number" name="latitude" id="latitude" step="any" value="{{ old('latitude', $item->latitude) }}" class="w-full admin-form-input" placeholder="-24.6532">
                            @error('latitude')
                                <p class="mt-1 text-sm text-[#DC2626]">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <input type="number" name="longitude" id="longitude" step="any" value="{{ old('longitude', $item->longitude) }}" class="w-full admin-form-input" placeholder="25.9087">
                            @error('longitude')
                                <p class="mt-1 text-sm text-[#DC2626]">{{ $message }}</p>
                            @enderror
                        </div>
                    ...
                    <div>
                        ...
                        <input type="text" name="location_address" id="location_address" value="{{ old('location_address', $item->location_address) }}" class="w-full admin-form-input" placeholder="123 Main St, Gaborone, Botswana">
                ...
                <div class="bg-admin-surface-alt border border-admin-border rounded-xl p-6 space-y-4">
                    ...
                    <div>
                        ...
                        <input type="number" name="sort_order" id="sort_order" value="{{ old('sort_order', $item->sort_order) }}" min="0" class="w-full admin-form-input" placeholder="Lower numbers appear first">
                        @error('sort_order')
                            <p class="mt-1 text-sm text-[#DC2626]">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="flex items-center gap-3 p-4 bg-admin-surface-alt rounded-xl border border-admin-border">
                        ...
                        <input type="checkbox" name="is_featured" id="is_featured" value="1" {{ old('is_featured', $item->is_featured) ? 'checked' : '' }} class="h-5 w-5 bg-admin-input-bg border-admin-input-border rounded focus:ring-2 focus:ring-[#DC2626] cursor-pointer">
                    ...
                    <div class="flex items-center gap-3 p-4 bg-admin-surface-alt rounded-xl border border-admin-border">
                        ...
                        <input type="checkbox" name="is_active" id="is_active" value="1" {{ old('is_active', $item->is_active) ? 'checked' : '' }} class="h-5 w-5 bg-admin-input-bg border-admin-input-border rounded focus:ring-2 focus:ring-[#DC2626] cursor-pointer">
