<x-layouts::admin title="Create User">
    <div class="p-6">
        ...
        {{-- Title Section --}}
        <div class="mb-8">
            <h1 class="font-headline text-3xl font-bold text-admin-text tracking-tight">Create User</h1>
            <p class="text-admin-text-muted text-sm mt-2">Add a new user account</p>
        </div>
        ...
        <form method="POST" action="{{ route('admin.users.store') }}" class="max-w-2xl">
            ...
            <div class="admin-glass-card rounded-3xl shadow-black/20 p-6 space-y-5">
                <div>
                    ...
                    <input type="text" name="name" id="name" required class="admin-form-input w-full" placeholder="Full name">
                    @error('name')
                ...
                <div>
                    ...
                    <input type="email" name="email" id="email" required class="admin-form-input w-full" placeholder="email@example.com">
                    @error('email')
                        <p class="mt-2 text-sm text-admin-accent">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    <label for="password" class="block text-xs font-semibold text-admin-text-muted uppercase tracking-wider mb-2">Password</label>
                    <input type="password" name="password" id="password" required class="admin-form-input w-full" placeholder="Secure password">
                    @error('password')
                        <p class="mt-2 text-sm text-admin-accent">{{ $message }}</p>
                    @enderror
                </div>
