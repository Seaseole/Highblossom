<x-layouts::admin title="Edit SEO">
    <div class="p-6">
        <div class="admin-section-header">
            <h1 class="admin-section-title">Edit SEO: {{ $route_label }}</h1>
        ...
        <form method="POST" action="{{ route('admin.seo.update', $route->id) }}" class="max-w-2xl">
            @csrf
            @method('PUT')
            ...
            <div class="bg-admin-surface-alt border border-admin-border rounded-xl p-6 space-y-6">
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <input type="text" name="meta_title" id="meta_title" value="{{ old('meta_title', $route->meta_title) }}" maxlength="70" class="w-full admin-form-input" placeholder="Page title">
                        <p class="text-sm text-admin-text-muted mt-1">{{ strlen(old('meta_title', $route->meta_title ?? '')) }}/70 characters</p>
                    </div>

                    <div>
                        <label for="meta_keywords" class="block text-sm font-medium text-admin-text-muted mb-2">Meta Keywords</label>
                        <input type="text" name="meta_keywords" id="meta_keywords" value="{{ old('meta_keywords', $route->meta_keywords) }}" class="w-full admin-form-input" placeholder="keyword1, keyword2, keyword3">
                    </div>

                    <div>
                        <label for="meta_description" class="block text-sm font-medium text-admin-text-muted mb-2">Meta Description</label>
                        <textarea name="meta_description" id="meta_description" rows="3" maxlength="300" class="w-full admin-form-input resize-none" placeholder="Brief description for search results">{{ old('meta_description', $route->meta_description) }}</textarea>
                        <p class="text-sm text-admin-text-muted mt-1">{{ strlen(old('meta_description', $route->meta_description ?? '')) }}/300 characters</p>
                    </div>
                </div>
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <input type="text" name="og_title" id="og_title" value="{{ old('og_title', $route->og_title) }}" maxlength="70" class="w-full admin-form-input" placeholder="Social sharing title">
                    </div>

                    <div>
                        <label for="og_image" class="block text-sm font-medium text-admin-text-muted mb-2">OG Image URL</label>
                        <input type="text" name="og_image" id="og_image" value="{{ old('og_image', $route->og_image) }}" class="w-full admin-form-input" placeholder="https://example.com/image.jpg">
                    </div>

                    <div>
                        <label for="og_description" class="block text-sm font-medium text-admin-text-muted mb-2">OG Description</label>
                        <textarea name="og_description" id="og_description" rows="3" maxlength="300" class="w-full admin-form-input resize-none" placeholder="Description for social media sharing">{{ old('og_description', $route->og_description) }}</textarea>
                    </div>
                </div>
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <input type="text" name="twitter_title" id="twitter_title" value="{{ old('twitter_title', $route->twitter_title) }}" maxlength="70" class="w-full admin-form-input" placeholder="Twitter sharing title">
                    </div>

                    <div>
                        <label for="twitter_image" class="block text-sm font-medium text-admin-text-muted mb-2">Twitter Image URL</label>
                        <input type="text" name="twitter_image" id="twitter_image" value="{{ old('twitter_image', $route->twitter_image) }}" class="w-full admin-form-input" placeholder="https://example.com/image.jpg">
                    </div>

                    <div>
                        <label for="twitter_description" class="block text-sm font-medium text-admin-text-muted mb-2">Twitter Description</label>
                        <textarea name="twitter_description" id="twitter_description" rows="3" maxlength="300" class="w-full admin-form-input resize-none" placeholder="Description for Twitter cards">{{ old('twitter_description', $route->twitter_description) }}</textarea>
                    </div>
                </div>
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <input type="text" name="canonical_url" id="canonical_url" value="{{ old('canonical_url', $route->canonical_url) }}" class="w-full admin-form-input" placeholder="https://example.com/page">
                    </div>

                    <div>
                        <label for="robots" class="block text-sm font-medium text-admin-text-muted mb-2">Robots</label>
                        <input type="text" name="robots" id="robots" value="{{ old('robots', $route->robots) }}" class="w-full admin-form-input" placeholder="index, follow">
                    </div>
                    ...
                    <div>
                        ...
                        <input type="number" name="priority" id="priority" step="0.1" min="0" max="1" value="{{ old('priority', $route->priority) }}" class="w-full admin-form-input" placeholder="0.5">
                    </div>

                    <div class="flex items-center gap-3 p-4 bg-admin-surface-alt rounded-xl border border-admin-border">
                        <input type="checkbox" name="no_index" id="no_index" value="1" {{ old('no_index', $route->no_index) ? 'checked' : '' }} class="h-5 w-5 bg-admin-input-bg border-admin-border rounded focus:ring-2 focus:ring-[#DC2626] cursor-pointer">
