        ...
        <div class="flex flex-col md:flex-row md:items-end justify-between gap-6">
            <div class="space-y-1">
                <div class="flex items-center gap-3 text-admin-text-muted mb-2">
                    <a href="{{ route('admin.gallery-categories.index') }}" class="hover:text-admin-accent transition-colors">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M10 19l-7-7m0 0l7-7m-7 7h18"/>
                        </svg>
                    </a>
                    <span class="text-[10px] font-bold uppercase tracking-[0.2em] font-body">Gallery Categories / Edit</span>
                </div>
                ...
                <p class="text-admin-text-muted text-sm max-w-lg">
                    Modify the architectural category details and visibility settings.
                </p>
            </div>
        </div>

        <form method="POST" action="{{ route('admin.gallery-categories.update', $galleryCategory) }}" class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            @csrf
            @method('PUT')
                ...
                <div class="admin-glass-card p-8 rounded-3xl shadow-2xl shadow-black/20 space-y-8">
                    <div class="space-y-6">
                        <div class="grid grid-cols-1 gap-6">
                            <div class="space-y-2">
                                <label for="name" class="text-[10px] font-bold text-admin-text-muted uppercase tracking-[0.2em] font-body ml-1">Category Name</label>
                                <input type="text" name="name" id="name" value="{{ old('name', $galleryCategory->name) }}" required class="admin-form-input font-headline text-lg font-bold tracking-tight" placeholder="e.g. Architectural Aluminum">
                                @error('name')
            ...
            <div class="space-y-6">
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="grid grid-cols-1 gap-4">
                            <div class="space-y-2">
                                ...
                                <input type="number" name="sort_order" id="sort_order" value="{{ old('sort_order', $galleryCategory->sort_order) }}" class="admin-form-input text-sm" placeholder="0">
                                @error('sort_order')
                                    <p class="mt-1 text-[10px] font-bold text-admin-accent uppercase tracking-wider">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div class="p-4 bg-admin-surface-alt/30 rounded-xl border border-admin-border-subtle group hover:border-admin-accent/20 transition-colors">
                                <label class="ui-checkbox-wrapper flex items-center gap-3 cursor-pointer">
                                    ...
                                    <input type="checkbox" name="is_active" id="is_active" value="1" class="ui-checkbox-input" {{ old('is_active', $galleryCategory->is_active) ? 'checked' : '' }}>
                                    ...
                                    <div class="flex flex-col">
                                        ...
                                        <span class="text-[10px] text-admin-text-muted font-body">Visible in gallery filters</span>
                                    </div>
                                </label>
                            </div>
                        </div>
                ...
                <div class="flex flex-col gap-3">
                    <button type="submit" class="admin-action-btn admin-action-btn-primary w-full justify-center py-4 shadow-lg shadow-admin-accent/20 focus:ring-admin-accent">
                        <span class="text-xs font-bold uppercase tracking-[0.2em]">Update Category</span>
                    </button>
                    <a href="{{ route('admin.gallery-categories.index') }}" class="admin-action-btn admin-action-btn-ghost w-full justify-center py-3">
