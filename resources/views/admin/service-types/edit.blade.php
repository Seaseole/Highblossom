<x-layouts::admin title="Edit Service Type">
    <div class="p-6">
        <div class="admin-section-header">
            <h1 class="admin-section-title">Edit Service Type</h1>
            <a href="{{ route('admin.service-types.index') }}" class="admin-action-btn admin-action-btn-secondary">
        ...
        <form method="POST" action="{{ route('admin.service-types.update', $serviceType) }}" class="max-w-2xl">
            @csrf
            @method('PUT')
            ...
            <div class="bg-admin-surface-alt border border-admin-border rounded-xl p-6 space-y-4">
                <div>
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name', $serviceType->name) }}" required class="w-full admin-form-input" placeholder="e.g. Replacement">
                    @error('name')
                ...
                <div>
                    ...
                    <input type="number" name="sort_order" id="sort_order" value="{{ old('sort_order', $serviceType->sort_order) }}" class="w-full admin-form-input" placeholder="Lower numbers appear first">
                    @error('sort_order')
                        <p class="mt-1 text-sm text-[#DC2626]">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="flex items-center gap-3 p-4 bg-admin-surface-alt rounded-xl border border-admin-border">
                    ...
                    <input type="checkbox" name="is_active" id="is_active" value="1" {{ $serviceType->is_active ? 'checked' : '' }} class="h-5 w-5 bg-admin-input-bg border-admin-input-border rounded focus:ring-2 focus:ring-[#DC2626] cursor-pointer">
