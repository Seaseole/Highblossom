...
<section class="{{ $containerClass }} py-12 px-6 rounded-xl my-8">
    <div class="mx-auto max-w-2xl text-center">
        <h3 class="mb-3 text-2xl font-bold">{{ $heading }}</h3>
        @if ($description)
            <p class="mb-6 text-lg opacity-90">{{ $description }}</p>
        @endif

        <form action="{{ route('contact') }}" method="GET" class="{{ $formClass }}">
            <input
                type="email"
                name="email"
        ...
        <p class="mt-4 text-xs opacity-70">{{ __('We respect your privacy. Unsubscribe at any time.') }}</p>
    </div>
</section>
