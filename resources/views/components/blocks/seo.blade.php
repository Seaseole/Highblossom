...
@if ($schema_type === 'FAQPage' && ! empty($faq_items))
    <section class="my-8">
        <h2 class="mb-6 text-2xl font-bold text-gray-900 dark:text-white">{{ __('Frequently Asked Questions') }}</h2>
        <div class="space-y-4">
            @foreach ($faq_items as $item)
                <details class="group rounded-lg bg-gray-50 dark:bg-gray-800">
                    ...
                    <div class="px-4 pb-4 text-gray-600 dark:text-gray-300">{{ $item['answer'] }}</div>
                </details>
            @endforeach
        </div>
    </section>
