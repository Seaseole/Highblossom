...
<div {{ $attributes->merge(['class' => "relative inline-flex items-center {$gap} w-fit"]) }}>
    {{-- Icon mark (left) --}}
    @if ($iconSrc)
        <img src="{{ $iconSrc }}" alt="{{ $businessName }} icon" class="h-10 w-10 rounded-lg object-contain" />
    @else
    ...
    {{-- Logo text --}}
    <span class="{{ $fontSize }} font-semibold tracking-tight leading-none"> {{ $businessName }} </span>
