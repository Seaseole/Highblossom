<div
    ...
    init() {
        <?php if(session('success')): ?>
            this.addToast('success', <?php echo \Illuminate\Support\Js::from(session('success'))->toHtml() ?>);
        <?php endif; ?>
        <?php if(session('error')): ?>
            this.addToast('error', <?php echo \Illuminate\Support\Js::from(session('error'))->toHtml() ?>);
        <?php endif; ?>
        <?php if(session('info')): ?>
            this.addToast('info', <?php echo \Illuminate\Support\Js::from(session('info'))->toHtml() ?>);
        <?php endif; ?>
        <?php if(session('warning')): ?>
            this.addToast('warning', <?php echo \Illuminate\Support\Js::from(session('warning'))->toHtml() ?>);
        <?php endif; ?>
        <?php if($errors->any()): ?>
            <?php $__currentLoopData = $errors->all(); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $error): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                this.addToast('error', <?php echo \Illuminate\Support\Js::from($error)->toHtml() ?>);
...
>
    <template x-for="toast in toasts" :key="toast.id">
        <div
            ...
            class="pointer-events-auto relative overflow-hidden rounded-xl border bg-white/95 shadow-2xl backdrop-blur-xl dark:bg-[#1E1E2E]/95"
            :class="{
...
<?php /**PATH C:\laragon\www\Highblossom\resources\views/components/ui/toaster.blade.php ENDPATH**/ ?>